...
extension InvoiceView {
    ...
    func populateData(invoice:Invoice?){
        self.specialityFeesValueLabel.text = currency + "\(invoice?.speciality_fees ?? 0)"
        self.categoryFeesValueLabel.text = currency + "\(invoice?.consulting_fees ?? 0)"
        if invoice?.discount == 0{
            self.discountView.isHidden = true
        }else{
        self.discountValueLabel.text =  currency + "\(invoice?.discount ?? 0)"
            self.discountView.isHidden = false
        }
        self.grossValueLabel.text =  currency + "\(invoice?.gross ?? 0)"
        self.totalValueLabel.text = currency + "\(invoice?.total_pay ?? 0)"
    }
